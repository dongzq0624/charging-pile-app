<template>
	<view class="whitePage" style="background-color: #FFFCF4;">
		<jcode-navbar title="充值" leftIcon="left"></jcode-navbar>
		...
		<view class="walletcard">
			...
			<view style="font-size: 55rpx;font-weight: bold;margin-top: 10rpx;z-index: 1;">{{numFilter(memberWallet.amount,2)}}</view>
			<view
				style="font-size: 30rpx;display: flex;justify-content: space-around;width: 100%;align-items: center;margin-top: 10rpx;z-index: 1;">
				<view @click="toWalletRecord(0)">扣减记录</view>
				<span style="font-size: 24rpx;">|</span>
				<view @click="toWalletRecord(1)">充值记录</view>
				<span style="font-size: 24rpx;">|</span>
				<view @click="toWalletRecord(2)">退款记录</view>
			</view>
		</view>
		...
		<view class="rechargecard">
			...
			<view
				...
				<input style="margin-left: 20rpx;font-size: 80rpx;width: 70%;font-weight: bold;height: 80%;" type="digit" v-model="money"
					@input="checkNum" :maxlength="maxlength" @blur="blur" @focus="focus" placeholder="自定义金额" placeholder-style="font-weight: 300;font-size:40rpx;"/>
			</view>
...
<script setup>
	...
	const selectMoney = (index) => {
		...
		// if (index == -1) {
		// 	endChargeMoney.value = (money.value == null || money.value == '') ? 0 : money.value
		// } else {
		// 	console.log('moneyList.value', moneyList.value)
		// 	endChargeMoney.value = moneyList.value[index].money
		// }
	}
